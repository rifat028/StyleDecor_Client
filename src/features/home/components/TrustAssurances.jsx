...
const TrustAssurances = () => {
  return (
    <section className="relative py-20 px-4 md:px-8 lg:px-12 bg-slate-50/60 dark:bg-gray-950 transition-colors duration-300 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto space-y-14">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300 text-sm font-semibold tracking-wide border border-purple-200 dark:border-purple-800/50 shadow-xs">
            ...
            <span>🛡️ Peace of Mind Guaranteed</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {TRUST_PILLARS.map((pillar) => {
            ...
            return (
              <div
                key={pillar.id}
                className={`relative group bg-white/90 dark:bg-gray-900/90 backdrop-blur-md rounded-2xl p-6 sm:p-7 border border-purple-100/80 dark:border-purple-900/40 shadow-md hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 flex flex-col justify-between overflow-hidden ${pillar.hoverGlow}`}
              >
                {/* Top Subtle Card Highlight Line */}
                <div
                  className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r ${pillar.gradient} opacity-80 group-hover:opacity-100 transition-opacity`}
                />

                <div>
                  {/* Glowing Top Icon Container & Badge Header */}
                  <div className="flex items-center justify-between mb-5">
                    <div className="p-3.5 bg-gradient-to-br from-purple-100 via-indigo-50 to-amber-100 dark:from-purple-950 dark:via-indigo-950 dark:to-purple-900 text-purple-700 dark:text-purple-300 rounded-2xl w-fit border border-purple-200/60 dark:border-purple-800/60 shadow-xs group-hover:scale-110 transition-transform duration-300">
                      <Icon className="w-6 h-6 text-purple-600 dark:text-purple-400" />
                    </div>

                    <span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-slate-100 dark:bg-slate-800 text-gray-700 dark:text-gray-300 border border-slate-200 dark:border-slate-700">
                      {pillar.badgeText}
                    </span>
                  </div>
                ...
                <div className="pt-6 mt-6 border-t border-gray-100 dark:border-gray-800/80 flex items-center gap-2 text-xs font-semibold text-gray-500 dark:text-gray-400">
        ...
        <div className="relative rounded-2xl bg-gradient-to-r from-purple-900 via-indigo-900 to-slate-900 text-white p-5 sm:p-6 shadow-xl border border-purple-800/40 flex flex-col md:flex-row items-center justify-between gap-4">
