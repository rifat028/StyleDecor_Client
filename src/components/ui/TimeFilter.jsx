...
/**
 * Standardized Reusable Time Period Filter Component
 *
 * @param {string} [timeFilter="max"] - Active filter ID
 * @param {Function} onTimeFilterChange - Callback when a preset is chosen or custom date applied
 * @param {string} [startDate=""] - Custom start date (YYYY-MM-DD)
 * @param {string} [endDate=""] - Custom end date (YYYY-MM-DD)
 * @param {Function} [onCustomDateChange] - Callback (startDate, endDate) when custom dates are applied
 * @param {Function} [onRefresh] - Optional callback to render the Refresh Data button
 * @param {boolean} [showRefresh=true] - Whether to show the Refresh button
 * @param {boolean} [loading=false] - Loading state for refresh spinner
 * @param {string} [align="right"] - Dropdown alignment ('right' | 'left')
        ...
        {isOpen && (
          <>
            ...
            <div
              ...
              } mt-2 w-52 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl z-50 py-1.5 animate-in fade-in zoom-in-95 duration-150`}
            >
