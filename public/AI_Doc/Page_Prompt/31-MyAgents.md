...
- [ ] Decorator profile still resolves correctly via the shared hook, matching prior behavior for both the success and email-fallback paths, with no duplicate lookup logic remaining in this file.
- [ ] An agent suspended by an admin (`ManageAgents.page.jsx`) shows a status this page's Edit modal can actually display and select, not a value missing from the dropdown.
- [ ] Every selectable/filterable agent status has a distinct, non-gray badge treatment.
- [ ] Page renders and behaves identically after extracting the 3 modals and agent card into local sub-components.
