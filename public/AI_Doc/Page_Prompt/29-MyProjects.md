...
- [ ] Status badges render identically (visually) after extraction into the shared component, matching `MyBookings.page.jsx` and `ManageBookings.page.jsx`.
- [ ] Decorator profile still resolves correctly via the shared hook, matching prior behavior for both the `/decorators/me` success path and the email-fallback path.
- [ ] Action button colors match the rest of the page's purple theme.
- [ ] Page renders and behaves identically after extracting the 3 modals and table row into local sub-components.
